...
[![codecov](https://codecov.io/gh/mgaidamak/cinema/branch/master/graph/badge.svg)](https://codecov.io/gh/mgaidamak/cinema)
...
20. отказаться от передачи переменных окружения через доллар. передавать по оригинальным путям в hocon.
...
* Сортировка записей не предполагается, все отдается с сортировкой по
первичному ключу (просто, чтобы не усложнять API).
* Продолжительность сеанса в базе не хранится. Можно доработать в будущем.
